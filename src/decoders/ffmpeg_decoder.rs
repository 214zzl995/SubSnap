...
impl FFmpegDecoder {
    ...
    fn extract_yuv_data_optimized(&mut self, decoded: &ffmpeg::util::frame::video::Video) -> Result<Vec<u8>> {
        if decoded.format() == ffmpeg::util::format::Pixel::YUV420P {
            ...
            // 从内存池获取预分配的缓冲区
            let mut frame_data = self.pool.get_buffer(total_size);
            ...
            let mut frame_data = self.pool.get_buffer(data_size);
            unsafe {
        ...
        for (stream, packet) in input.packets() {
            if stream.index() == video_stream_index && frame_count < final_output_frames {
                ...
                while decoder.receive_frame(&mut decoded).is_ok() && frame_count < final_output_frames {
                    ...
                    let yuv_data = self.extract_yuv_data_optimized(&decoded)?;
